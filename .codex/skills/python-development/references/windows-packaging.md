...
配置文件默认与脚本同名 `脚本名.ini`，使用 `configparser`。

- 读取：UTF-8 优先，失败后回退 GB2312；编码回退只处理解码失败（`UnicodeDecodeError`），文件不存在、权限不足等问题直接抛出，不要被 fallback 掩盖。
- 写入：明确指定 `encoding="utf-8"`。
- 不要把已存在的 GB2312 配置静默改写为 UTF-8；确需改写时保持原编码或先备份。
- 配置不存在时根据需求创建并停止运行，配置项使用中文注释。

## 编码

文本读取优先 UTF-8，涉及旧 Windows/中文本地文件时考虑 GB2312/GBK。读取失败时是否 fallback 应根据数据来源决定；INI 与 txt/csv 等文本遵循同一条回退规则（UTF-8 优先、GB2312 备选）。不要用 `errors="ignore"` 掩盖未知编码问题，可能静默丢数据。写文件必须明确指定 encoding。
